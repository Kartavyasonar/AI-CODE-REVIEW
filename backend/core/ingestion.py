...
FIXES applied:
  - ChromaDB version compatibility fix: use PersistentClient with temp dir
    to avoid "default_tenant" connection error in ChromaDB 0.5.x
        ...
        for fname in filenames:
            ...
            try:
                ...
                if len(content.strip()) > 20:
                    files.append({"path": rel, "content": content, "language": ext.lstrip(".")})
            except Exception:
                pass
...
def chunk_python_file(content: str, filepath: str) -> list:
    ...
    for node in ast.walk(tree):
        ...
        if len(chunk_text.strip()) < 30:
            continue
        chunks.append({
            "id": str(uuid.uuid4()), "content": chunk_text[:MAX_CHUNK_CHARS],
            "filepath": filepath, "type": type(node).__name__, "name": node.name,
            "start_line": node.lineno, "end_line": end, "language": "py",
        })
...
def chunk_naive(content: str, filepath: str, language: str) -> list:
    ...
    for i in range(0, max(1, len(content)), step):
        ...
        if len(chunk_text.strip()) < 30:
            continue
        chunks.append({
            "id": str(uuid.uuid4()), "content": chunk_text, "filepath": filepath,
            "type": "chunk", "name": f"chunk_{i}", "start_line": None,
            "end_line": None, "language": language,
        })
    return chunks
...
def ingest_repo(repo_url: str):
    ...
    collection, chroma_temp_dir = build_vector_store(all_chunks, collection_name)
    return collection, all_chunks, repo_path, chroma_temp_dir
